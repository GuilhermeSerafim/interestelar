...
const Team = (props) => {
    const { name, primaryColor, backgroundColor, astrosFiltrados } = props;
console.log(astrosFiltrados)
    return (
        astrosFiltrados.length > 0 ? <section className="team" style={{ backgroundColor: backgroundColor }}> {/**2 chaves - 1 para incorporar js, e a 2 é sintaxe para objeto js */}
            ...
            <div className="astros">
                {astrosFiltrados.map(astro => <AstroCard key={astro.nome} nome={astro.nome} planeta={astro.planeta} imagem={astro.imagem} habilidades={astro.habilidades} primaryColor={primaryColor} />)}
            </div>
        </section>
